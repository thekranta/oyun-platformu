...
// ============================================
// ⚖️ EN AĞIR HANGİSİ? - Ağırlık karşılaştırma (Matematik/MAB.3)
// İki nesneden hangisinin daha ağır olduğunu seçer. Ölçme/karşılaştırma;
// bariz farklı ikililer (fil-fare). Görsel + günlük bilgi.
// ============================================
...
export default function AgirHafif({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    ...
    const built: Card[] = shuffle([
      { id: 0, emoji: pair.h, heavy: true },
      { id: 1, emoji: pair.l, heavy: false },
    ]);
    setCards(built);
    setLocked(false);
    setWrongId(null);
  ...
  const finish = () => {
    ...
    onGameEnd('agir-hafif', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Matematik: Ağırlığa Göre Karşılaştırma (en ağır) (MAB.3)',
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="İki nesneden hangisi daha ağır? Ağır olana dokun!"
          childName={childName}
          countdownSeconds={5}
      ...
      <TouchableOpacity style={styles.listenBtn} onPress={() => speak('En ağır olanı bul!', { instructions: HAPPY_VOICE })} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F5F0EC', alignItems: 'center' },
  ...
  listenBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#4E342E', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 22, marginTop: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 1, elevation: 3 },
