...
// Akıllı Çıkarma — UYARLANIR (adaptif) zorluk. Maarif: MAB.7
// (Matematiksel problemler ve çözümlerine ilişkin stratejiler geliştirebilme).
// "Kaç tane kaldı?": bazı nesneler gitti (soluk), kalanları say. Zorlukla sayılar büyür.
...
interface Round { total: number; removed: number; answer: number; options: number[]; }

function buildRound(diff: number): Round {
    const c = CFG[diff] || CFG[2];
    const total = 2 + Math.floor(Math.random() * (c.max - 1)); // en az 2
    const removed = 1 + Math.floor(Math.random() * (total - 1)); // 1..total-1
    const answer = total - removed;

    const opts = new Set<number>([answer]);
    if (c.near) {
        [answer - 1, answer + 1, answer + 2].filter(v => v >= 0).forEach(v => opts.add(v));
    }
    let guard = 0;
    while (opts.size < 4 && guard++ < 40) {
        const v = Math.max(0, answer + (Math.floor(Math.random() * 7) - 3));
        opts.add(v);
    }
    const options = shuffle(Array.from(opts)).slice(0, 4);
    if (!options.includes(answer)) options[0] = answer;
    return { total, removed, answer, options: shuffle(options) };
}

export default function AkilliCikarma({ onGameEnd, onExit, childName = 'Küçük Kaşif' }: Props) {
    const START_DIFF = 2;
    ...
    const nextRound = useCallback((solvedInDiff: number) => {
        ...
        if (round >= TOTAL_ROUNDS) {
            ...
            onGameEnd('akilli-cikarma', duration, totalMovesRef.current, totalErrorsRef.current, undefined, {
                zorlukSeviyesi: solvedInDiff,
                kazanimOdagi: 'MAB.7 Çıkarma / problem çözme stratejisi (uyarlanır zorluk)',
            });
            return;
        }
        ...
        <DynamicBackground>
            <View style={styles.container}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.title}>📈 Akıllı Çıkarma</Text>
                ...
                <Animated.View style={[styles.card, { transform: [{ scale: bump }, { translateX: shake }] }]}>
                    <View style={styles.objectsWrap}>
                        {Array.from({ length: current.total }, (_, i) => {
                            const gone = i >= current.total - current.removed;
                            return (
                                <View key={i} style={styles.objSlot}>
                                    <Text style={[styles.object, gone && styles.objGone]}>{OBJECT}</Text>
                                    {gone && <Text style={styles.cross}>✖️</Text>}
                                </View>
                            );
                        })}
                    </View>
                    <Text style={styles.caption}>✖️ olanlar gitti — kalanları say</Text>
                </Animated.View>
...
const styles = StyleSheet.create({
    ...
    card: {
        ...
        padding: 14, minHeight: 150, width: '100%', maxWidth: 480, justifyContent: 'center',
    ...
    objSlot: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center' },
    object: { fontSize: 36 },
    objGone: { opacity: 0.22 },
    cross: { position: 'absolute', fontSize: 20 },
    caption: { fontSize: 12, color: '#8D6E63', textAlign: 'center', marginTop: 10, fontStyle: 'italic' },
    ...
    hint: { fontSize: 15, color: '#5D4037', marginTop: 16, fontWeight: '600', minHeight: 22, textAlign: 'center' },
