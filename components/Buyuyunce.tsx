...
// Küçük canlı büyüyünce neye dönüşür? (tırtıl→kelebek). Canlıların büyüme
// ve değişimini gözlemleme. Görsel bilgi + akıl yürütme.
// ============================================
...
export default function Buyuyunce({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    if (!gameReady) return;
    const pool = PAIRS.filter((x) => x.y !== prevRef.current);
    const cur = pool[Math.floor(Math.random() * pool.length)];
    prevRef.current = cur.y;
    const distract = shuffle(GROWNS.filter((g) => g !== cur.g)).slice(0, 2);
    setPair(cur);
    setOptions(shuffle([cur.g, ...distract]));
    setLocked(false);
    setWrong(null);
    bounce.setValue(0.85);
    Animated.spring(bounce, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak('Bu büyüyünce ne olur?', { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('buyuyunce', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Fen: Canlıların Büyüme ve Değişimini Gözlemleme (yaşam döngüsü) (FAB.1)',
  ...
  const handlePick = (g: string) => {
    ...
    if (g === pair.g) {
      ...
      speakThenWait('Doğru! İşte büyümüş hali. Aferin.', 1400, { instructions: HAPPY_VOICE }).then(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Canlılar büyür ve değişir! Bu küçük canlı büyüyünce ne olur, bul."
          childName={childName}
          countdownSeconds={5}
      ...
      <Text style={styles.prompt}>Büyüyünce ne olur?</Text>
      <Animated.View style={[styles.youngCard, { transform: [{ scale: bounce }] }]}>
        <Text style={styles.youngEmoji}>{pair.y}</Text>
      ...
      <View style={styles.options}>
        {options.map((g) => {
          const isWrong = wrong === g;
          return (
            <Animated.View key={g} style={isWrong ? { transform: [{ translateX: shake }] } : undefined}>
              <TouchableOpacity style={[styles.optCard, isWrong && styles.optWrong]} onPress={() => handlePick(g)} activeOpacity={0.85}>
                <Text style={styles.optEmoji}>{g}</Text>
              </TouchableOpacity>
            </Animated.View>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  prompt: { fontSize: 20, fontWeight: '800', color: '#00695C', marginTop: 8 },
