...
// Nesneyi canlı mı cansız mı diye ayırır. Doğru Kutu'nun farklı kategori
// varyasyonu; temel fen kavramı. Görsel.
// ============================================
...
const BINS = [
  { key: 'canli', name: 'Canlı', icon: '🌱', color: '#66BB6A' },
  { key: 'cansiz', name: 'Cansız', icon: '🪑', color: '#78909C' },
];
...
export default function CanliCansiz({ onGameEnd, onExit, childName }: Props) {
  const [gameReady, setGameReady] = useState(false);
  const [itemIndex, setItemIndex] = useState(0);
  const [emoji, setEmoji] = useState('🐶');
  const [cat, setCat] = useState('canli');
  ...
  useEffect(() => {
    ...
    setEmoji(isLiving ? pick(LIVING) : pick(NONLIVING));
    setCat(isLiving ? 'canli' : 'cansiz');
    setLocked(false);
    setWrongKey(null);
    pop.setValue(0.7);
    Animated.spring(pop, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak('Bu canlı mı, cansız mı?', { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('canli-cansiz', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Fen: Canlı/Cansız Ayrımı ve Sınıflandırma (FAB.2)',
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Canlılar nefes alır, büyür! Her nesneyi canlı mı cansız mı diye ayır."
          childName={childName}
          countdownSeconds={5}
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.iconBtn} onPress={onExit} activeOpacity={0.8}>
          <Ionicons name="arrow-back" size={24} color="#33691E" />
        </TouchableOpacity>
        <View style={styles.roundBadge}><Text style={styles.roundText}>🌱 {Math.min(itemIndex + 1, TOTAL_ITEMS)}/{TOTAL_ITEMS}</Text></View>
        <View style={{ width: 44 }} />
      </View>

      <Text style={styles.prompt}>Canlı mı, cansız mı?</Text>

      <TouchableOpacity style={styles.listenBtn} onPress={() => speak('Bu canlı mı, cansız mı?', { instructions: HAPPY_VOICE })} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  prompt: { fontSize: 21, fontWeight: '900', color: '#33691E', marginTop: 10 },
  listenBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#66BB6A', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 22, marginTop: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 1, elevation: 3 },
