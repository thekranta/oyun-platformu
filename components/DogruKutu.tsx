...
// Nesne belirir; çocuk onu türüne göre doğru kutuya koyar (Hayvan / Yiyecek).
// Anlamsal sınıflandırma (renge göre değil türe göre). Görsel.
// ============================================
...
const BINS = [
  { key: 'hayvan', name: 'Hayvanlar', icon: '🐾', color: '#66BB6A' },
  { key: 'yiyecek', name: 'Yiyecekler', icon: '🍽️', color: '#FFA726' },
];
...
export default function DogruKutu({ onGameEnd, onExit, childName }: Props) {
  const [gameReady, setGameReady] = useState(false);
  const [itemIndex, setItemIndex] = useState(0);
  const [emoji, setEmoji] = useState('🐶');
  const [cat, setCat] = useState('hayvan');
  ...
  useEffect(() => {
    ...
    setEmoji(isAnimal ? pick(ANIMALS) : pick(FOODS));
    setCat(isAnimal ? 'hayvan' : 'yiyecek');
    setLocked(false);
    setWrongKey(null);
    pop.setValue(0.7);
    Animated.spring(pop, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak('Bunu doğru kutuya koy!', { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('dogru-kutu', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Fen: Nesneleri Türüne Göre Sınıflandırma (hayvan/yiyecek) (FAB.2)',
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Her nesneyi doğru kutuya koy: hayvanlar bir kutuya, yiyecekler diğerine!"
          childName={childName}
          countdownSeconds={5}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F6F3EF', alignItems: 'center' },
