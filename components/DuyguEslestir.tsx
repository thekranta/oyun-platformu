...
// Karışık yüzler içinde AYNI duyguya sahip iki yüzü bul. Duygu tanıma +
// eşleştirme (Duygu Yüzleri'nin eşleştirme varyasyonu). Görsel.
// ============================================
...
interface Card { id: number; emoji: string; name: string; isTwin: boolean }
...
export default function DuyguEslestir({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    ...
    const twin = pool[0];
    const distractors = pool.slice(1, n - 1);
    const built: Card[] = [
      { id: 0, emoji: twin.emoji, name: twin.name, isTwin: true },
      { id: 0, emoji: twin.emoji, name: twin.name, isTwin: true },
      ...distractors.map((f) => ({ id: 0, emoji: f.emoji, name: f.name, isTwin: false })),
    ];
    ...
    setWrongIds([]);
    setLocked(false);
    speak('Aynı duyguya sahip iki yüzü bul!', { instructions: HAPPY_VOICE });
  }, [round, gameReady]);
  ...
  const finish = () => {
    ...
    onGameEnd('duygu-eslestir', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Sosyal-Duygusal: Aynı Duyguyu Tanıma ve Eşleştirme (TADB.2)',
  ...
  const handleTap = (card: Card) => {
    ...
    if (first.isTwin && card.isTwin) {
      ...
      speakThenWait(`İkisi de ${card.name}! Aferin.`, isLast ? 1500 : 1000, { instructions: HAPPY_VOICE }).then(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Yüzlere bak! Aynı duyguya sahip iki yüzü bul ve eşleştir."
          childName={childName}
          countdownSeconds={5}
