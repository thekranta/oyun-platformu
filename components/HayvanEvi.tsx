...
// ============================================
// 🏡 HAYVAN EVİ - Yaşam alanı eşleme (Bilişsel/Fen, FAB.2)
// Hayvanın NEREDE yaşadığını seç (balık→su). Canlıları özelliğine/yaşam
// alanına göre sınıflandırma. Görsel bilgi + akıl yürütme.
// ============================================
...
const ANIMALS = [
  { a: '🐟', h: 'su' }, { a: '🐳', h: 'su' }, { a: '🐙', h: 'su' },
  { a: '🦁', h: 'orman' }, { a: '🐒', h: 'orman' }, { a: '🐯', h: 'orman' },
  { a: '🐄', h: 'ciftlik' }, { a: '🐔', h: 'ciftlik' }, { a: '🐷', h: 'ciftlik' },
  { a: '🐧', h: 'kutup' }, { a: '🐻‍❄️', h: 'kutup' },
  { a: '🐝', h: 'bahce' }, { a: '🦋', h: 'bahce' },
  { a: '🦅', h: 'gokyuzu' }, { a: '🦜', h: 'gokyuzu' },
];
...
export default function HayvanEvi({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    if (!gameReady) return;
    const pool = ANIMALS.filter((x) => x.a !== prevRef.current);
    const cur = pool[Math.floor(Math.random() * pool.length)];
    prevRef.current = cur.a;
    const distract = shuffle(HAB_KEYS.filter((k) => k !== cur.h)).slice(0, 2);
    setAnimal(cur);
    setOptions(shuffle([cur.h, ...distract]));
    setLocked(false);
    setWrong(null);
    bounce.setValue(0.85);
    Animated.spring(bounce, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak('Bu hayvan nerede yaşar?', { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('hayvan-evi', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Fen: Canlıları Yaşam Alanına Göre Sınıflandırma (FAB.2)',
  ...
  const handlePick = (key: string) => {
    ...
    if (key === animal.h) {
      ...
      speakThenWait(`Doğru! ${HABITATS[key].loc} yaşar. Aferin.`, 1400, { instructions: HAPPY_VOICE }).then(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Her hayvanın bir evi var! Bu hayvan nerede yaşar, doğru yeri seç."
          childName={childName}
          countdownSeconds={5}
      ...
      <Text style={styles.prompt}>Nerede yaşar?</Text>
      <Animated.View style={[styles.animalCard, { transform: [{ scale: bounce }] }]}>
        <Text style={styles.animalEmoji}>{animal.a}</Text>
      </Animated.View>

      <TouchableOpacity style={styles.listenBtn} onPress={() => speak('Bu hayvan nerede yaşar?', { instructions: HAPPY_VOICE })} activeOpacity={0.85}>
          ...
          return (
            <Animated.View key={key} style={isWrong ? { transform: [{ translateX: shake }] } : undefined}>
              <TouchableOpacity style={[styles.optCard, isWrong && styles.optWrong]} onPress={() => handlePick(key)} activeOpacity={0.85}>
                <Text style={styles.optEmoji}>{HABITATS[key].emoji}</Text>
                <Text style={styles.optName}>{HABITATS[key].name}</Text>
              </TouchableOpacity>
            </Animated.View>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  animalCard: { width: 140, height: 140, borderRadius: 30, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', marginTop: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 10, elevation: 6 },
