...
// Hayvanın ne yediğini seç (tavşan→havuç). Canlılara dair bilgi; ilişki kurma.
// ============================================
...
const ALL_FOODS = PAIRS.map((p) => p.f);
...
export default function NeYer({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    if (!gameReady) return;
    const pool = PAIRS.filter((x) => x.a !== prevRef.current);
    const cur = pool[Math.floor(Math.random() * pool.length)];
    prevRef.current = cur.a;
    const distract = shuffle(ALL_FOODS.filter((f) => f !== cur.f)).slice(0, 2);
    setPair(cur);
    setOptions(shuffle([cur.f, ...distract]));
    setLocked(false);
    setWrong(null);
    bounce.setValue(0.85);
    Animated.spring(bounce, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak('Bu hayvan ne yer?', { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('ne-yer', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Fen: Canlıların Beslenmesi - Hayvan-Besin İlişkisi (FAB.2)',
      correct_answers: correctRef.current,
    });
  };

  const handlePick = (f: string) => {
    if (locked) return;
    movesRef.current += 1;
    if (f === pair.f) {
      setLocked(true);
      ...
      speak('Doğru! Aferin.', { instructions: HAPPY_VOICE });
      const t = setTimeout(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Her hayvanın sevdiği bir yiyecek var! Bu hayvan ne yer, doğru olanı seç."
          childName={childName}
          countdownSeconds={5}
      ...
      <Text style={styles.prompt}>Ne yer?</Text>
      ...
      <View style={styles.options}>
        {options.map((f) => {
          const isWrong = wrong === f;
          return (
            <Animated.View key={f} style={isWrong ? { transform: [{ translateX: shake }] } : undefined}>
              <TouchableOpacity style={[styles.optCard, isWrong && styles.optWrong]} onPress={() => handlePick(f)} activeOpacity={0.85}>
                <Text style={styles.optEmoji}>{f}</Text>
              </TouchableOpacity>
            </Animated.View>
          );
        })}
