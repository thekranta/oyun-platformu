...
// v1'in gelişmişi: daha büyük gridler, 6 renk, daha detaylı temalar
// (roket, balık, ağaç, ev, çiçek). Her karedeki sayının rengiyle boya.
// ============================================
...
const NUM_COLORS: Record<number, string> = {
  1: '#FF5A5A', 2: '#4FA3FF', 3: '#FFCE3A', 4: '#57D971', 5: '#B77BFF', 6: '#8D6E63',
};

// 0 = boş; 1-6 boyanacak kareler (o rengin numarası)
const TEMPLATES: { key: string; name: string; emoji: string; grid: string[] }[] = [
  { key: 'cicek', name: 'Çiçek', emoji: '🌸', grid: ['000333000', '003333300', '033333330', '033313330', '003333300', '000040000', '000040000', '004440000', '000040000'] },
  { key: 'agac', name: 'Ağaç', emoji: '🌳', grid: ['000444000', '004444400', '044414440', '044444440', '004444400', '000060000', '000060000', '000060000', '006666600'] },
  { key: 'balik', name: 'Balık', emoji: '🐟', grid: ['00000000004', '00000330000', '02222233000', '22212233300', '02222233000', '00000330000', '00000000000'] },
  { key: 'ev', name: 'Ev', emoji: '🏠', grid: ['000010003', '000111000', '001111100', '011111110', '033333330', '033323330', '033363330', '033363330', '033333330'] },
  { key: 'roket', name: 'Roket', emoji: '🚀', grid: ['000010000', '000111000', '001111100', '001121100', '001111100', '001111100', '011111110', '011111110', '110111011', '000666000', '000060000'] },
];
...
export default function SayiBoya2({ onGameEnd, onExit, childName }: Props) {
  ...
  const finish = () => {
    ...
    onGameEnd('sayi-boya-2', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 2,
      kazanimOdagi: 'Sanat/Matematik: Sayı-Renk Eşleme ile Detaylı Boyama (paint-by-number v2)',
  ...
  return (
    <View style={styles.container}>
      {showConfetti && <ConfettiCannon count={130} origin={{ x: SCREEN_W / 2, y: 0 }} fadeOut />}
      {!gameReady && (
        <CountdownOverlay
          message="Her karede bir sayı var! O sayının rengini seç ve kareye dokun, boya. Bakalım hangi resim çıkacak!"
          childName={childName}
          countdownSeconds={5}
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>🔢 Sayı-Boya 2</Text>
            ...
            {row.split('').map((ch, c) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  {!isFilled && <Text style={[styles.cellNum, { fontSize: cell * 0.42 }]}>{num}</Text>}
                </TouchableOpacity>
              );
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  picRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, paddingVertical: 6 },
  picBtn: { width: 46, height: 46, borderRadius: 14, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: 'transparent', elevation: 2 },
  ...
  cellNum: { fontWeight: '800', color: '#90A4AE' },

  palette: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 10, paddingVertical: 14, maxWidth: 460 },
  crayon: { width: 50, height: 50, borderRadius: 25, alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: 'rgba(0,0,0,0.12)', shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.2, shadowRadius: 1, elevation: 3 },
  crayonActive: { borderColor: '#212121', transform: [{ scale: 1.15 }] },
  crayonNum: { fontSize: 20, fontWeight: '900', color: '#fff', textShadowColor: 'rgba(0,0,0,0.25)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 1 },
});
