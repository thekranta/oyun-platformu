...
// ============================================
// 🔢 SAYIYI BUL - Rakam → nicelik (Matematik/MAB.1)
// Büyük bir rakam gösterilir; çocuk O KADAR nesne içeren grubu seçer.
// Sayı-nicelik ilişkisi (Çiftlikte Sayalım'ın tersi yönü). Görsel.
// ============================================
...
export default function SayiyiBul({ onGameEnd, onExit, childName }: Props) {
  ...
  const finish = () => {
    ...
    onGameEnd('sayiyi-bul', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Matematik: Rakam-Nicelik İlişkisi (MAB.1)',
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Büyük sayıya bak! O kadar nokta olan kutuyu bul."
          childName={childName}
          countdownSeconds={5}
      ...
      <Animated.View style={[styles.numCard, { transform: [{ scale: numBounce }] }]}>
        <Text style={[styles.numBig, { color }]}>{target}</Text>
      </Animated.View>

      <TouchableOpacity style={styles.listenBtn} onPress={() => speak(`${target} tane olanı bul.`, { instructions: HAPPY_VOICE })} activeOpacity={0.85}>
