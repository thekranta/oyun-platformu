...
// Duruma uygun temizlik aracını/eylemini seç. Temizlik ve düzen alışkanlığı.
// ============================================
...
interface Task { scene: string; text: string; good: string; distractors: string[] }

const TASKS: Task[] = [
  { scene: '🖐️', text: 'Ellerin kirlendi.', good: '🧼', distractors: ['📺', '⚽'] },
  { scene: '🦷', text: 'Yemek yedin, dişlerin.', good: '🪥', distractors: ['👟', '🎈'] },
  { scene: '💇', text: 'Saçların dağınık.', good: '🪮', distractors: ['🥄', '📚'] },
  { scene: '🤧', text: 'Burnun aktı.', good: '🧻', distractors: ['🔑', '🧸'] },
  { scene: '🧺', text: 'Oyuncaklar yerde dağınık.', good: '📦', distractors: ['🍭', '📱'] },
  { scene: '🛁', text: 'Çok kirlendin.', good: '🚿', distractors: ['🎮', '🚗'] },
];
...
export default function TemizlikZamani({ onGameEnd, onExit, childName }: Props) {
  ...
  useEffect(() => {
    if (!gameReady) return;
    if (orderRef.current.length === 0) orderRef.current = shuffle(TASKS);
    const tk = orderRef.current[(round - 1) % orderRef.current.length];
    setTask(tk);
    setOptions(shuffle([tk.good, ...tk.distractors]));
    setLocked(false);
    setWrong(null);
    bounce.setValue(0.85);
    Animated.spring(bounce, { toValue: 1, friction: 5, useNativeDriver: USE_NATIVE }).start();
    speak(`${tk.text} Ne kullanırız?`, { instructions: HAPPY_VOICE });
  ...
  const finish = () => {
    ...
    onGameEnd('temizlik-zamani', duration, movesRef.current, errorsRef.current, undefined, {
      zorlukSeviyesi: 1,
      kazanimOdagi: 'Hareket/Sağlık: Temizlik ve Düzen - Uygun Aracı Seçme (HSAB.9)',
  ...
  const handlePick = (emoji: string) => {
    ...
    if (emoji === task.good) {
      ...
      speakThenWait('Aferin! Tertemiz oldu.', 1400, { instructions: HAPPY_VOICE }).then(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!gameReady && (
        <CountdownOverlay
          message="Temiz ve düzenli olalım! Her durum için doğru şeyi seç."
          childName={childName}
          countdownSeconds={5}
...
const styles = StyleSheet.create({
  ...
  prompt: { fontSize: 20, fontWeight: '800', color: '#0277BD', marginTop: 8 },
